...
public class HW6Test extends GenericTest {
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("wordcount")) try {
      startTest("wordcount", "RDD.wordCount()", 0);
      try {
        Random r = new Random();
        int numLines = 20 + r.nextInt(5);
        String[] arg = new String[numLines];
        String[] words = {"A", "B", "C"};
        Map<String, Integer> wordCounts = new HashMap<>();

        for (int i = 0; i < numLines; i++) {
          int numWordsInLine = 1 + r.nextInt(5);
          StringBuilder lineBuilder = new StringBuilder();
          for (int j = 0; j < numWordsInLine; j++) {
            String word = words[r.nextInt(words.length)];
            lineBuilder.append(word);
            if (j < numWordsInLine - 1) {
              lineBuilder.append(" ");
            }
            wordCounts.put(word, wordCounts.getOrDefault(word, 0) + 1);
          }
          arg[i] = lineBuilder.toString();
        }

        String argsAsString = "(";
        for (int i = 0; i < arg.length; i++)
          argsAsString += ((i > 0) ? ", " : "") + "'" + arg[i] + "'";
        argsAsString += ")";

        List<String> expectedList = new ArrayList<>();
        for (Map.Entry<String, Integer> entry : wordCounts.entrySet()) {
          expectedList.add(entry.getKey() + ": " + entry.getValue());
        }
        ...
        if (response == null)
          testFailed("We submitted a job (flame-wordcount.jar) to the Flame coordinator, but it looks like the job failed. Here is the error output we got:\n\n" + FlameSubmit.getErrorResponse());

        List<String> responseLines = Arrays.asList(response.trim().split("\n"));
        Collections.sort(responseLines);
        String sortedResponse = String.join("\n", responseLines) + "\n";

        if (sortedResponse.equals(expected))
          testSucceeded();
        else
          testFailed("We sent " + argsAsString + " and expected to get:\n'" + expected + "', but we actually got:\n\n" + response);
      } catch (ConnectException ce) {
        testFailed("We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
      }
    } catch (Exception e) {
      testFailed("An exception occurred: " + e, false); e.printStackTrace();
    }
