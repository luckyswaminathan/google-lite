  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("count"))
      try {
        ...
        try {
          ...
          String response = FlameSubmit.submit("localhost:9000", "tests/hw7testjob.jar", "cis5550.test.HW7TestJob",
              arg);
          if (response.equals("" + num))
            testSucceeded();
          else
            testFailed(
                "We expected to get '" + num + "', but we actually got the following:\n\n" + dump(response.getBytes()));
        } catch (ConnectException ce) {
          testFailed(
              "We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
        }
      } catch (Exception e) {
        ...
        try {
          ...
          if (response.equals("OK")) {
            ...
            if (exception != null)
              ...
              testFailed(
                  "We created a table with " + num + " rows, but saveAsTable() returned one with " + cnt + " rows.");
            else if (nullForKey != null)
              testFailed("In the output table '" + arg[1] + "', we found a row with key '" + nullForKey
                  + "' that doesn't appear to have a 'value' column.");
            else if (extraValue != null)
              testFailed("In the output table '" + arg[1]
                  + "', we found an element (or an extra copy of an element) we didn't put into the RDD: '" + extraValue
                  + "'.");
            else
              testSucceeded();
          } else {
          ...
          if (pieces.length == takeNum) {
            ...
            if (missingPiece == null) {
              ...
              if (pieces.length == num)
                ...
                testFailed(
                    "We uploaded " + num + " elements to an RDD and then called take(30), so we expected to get all "
                        + num + " elements back, but instead we got " + pieces.length + " ('" + response + "')");
            } else {
              testFailed("We uploaded " + num + " elements ('" + theUpload + "') and then called take(" + arg[1]
                  + "), but we got '" + response + "', which doesn't look like a proper subset - for instance, '"
                  + missingPiece + "' is not in the original set");
            }
          } else {
            testFailed("We uploaded " + num + " elements to an RDD and then called take(" + arg[1] + "), but we got "
                + pieces.length + " elements back ('" + response + "')");
          }
        } catch (ConnectException ce) {
          testFailed(
              "We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
      ...
      try {
        ...
        try {
          ...
          if (response.equals(expected))
            ...
            testFailed("We put " + num + " elements ('" + expected + "') into table '" + tableName
                + "', then called fromTable('" + tableName + "') and collected the result, but we got '" + response
                + "'");
        } catch (ConnectException ce) {
          testFailed(
              "We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
      ...
      try {
        ...
        try {
          ...
          if (response.equals(expected))
            ...
            testFailed("We put " + num + " elements ('" + provided
                + "') into an RDD and then mapped them to pairs, with the first character serving as the key and the rest as the value; we expected\n\n"
      ...
      try {
        ...
        try {
          ...
          if (response.equals(expected))
            ...
            testFailed("We put " + (num / 2) + " elements ('" + provided
                + "') into a PairRDD and then mapped them to strings that were simply concatenations of the keys and values (for each pair, first key+value then value+key). We expected to get\n\n"
      ...
      try {
        ...
        try {
          ...
          if (response.equals(expected))
            ...
            testFailed("We put " + (num / 2) + " elements ('" + provided
                + "') into a PairRDD and then used flatMapToPair to output (key,length) and (value,length) for each. We expected to get\n\n"
                + expected + "\n\nbut we got back the following instead:\n\n" + dump(response.getBytes()));
        } catch (ConnectException ce) {
          testFailed(
              "We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
      ...
      try {
        ...
        try {
          ...
          if (response.equals(expected))
            ...
            testFailed("We put " + numUnique + " unique strings and " + numDuplicates + " duplicates ('" + provided
                + "') into an RDD and then called distinct(). We expected to get\n\n" + expected
                + "\n\nbut we got back the following:\n\n" + dump(response.getBytes()));
        } catch (ConnectException ce) {
          testFailed(
              "We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
      ...
      try {
        ...
        try {
          ...
          if (response.equals("" + sum))
            ...
            testFailed("We put " + num + " numbers ('" + provided
                + "') into an RDD and then called fold() to add them up. We expected the sum to be " + sum
                + ", but we got back the following:\n\n" + dump(response.getBytes()));
        } catch (ConnectException ce) {
          testFailed(
              "We were not able to connect to the Flame coordinator at localhost:9000. Verify that the coordinator is running and hasn't crashed?");
    ...
    if (tests.contains("filter"))
      try {
        ...
        try {
          // Generate random strings, ensuring some contain "a" for the predicate
          Random random = new Random();
          int numElements = 10 + random.nextInt(5);
          String[] arg = new String[1 + numElements];
          String provided = "";
          arg[0] = "filter";
          List<String> elemList = new LinkedList<String>();
          for (int i = 0; i < numElements; i++) {
            String elem = randomAlphaNum(5, 10);
            if (i % 2 == 0) {
              // Modify element to include "a" for filter predicate
              elem = "a" + elem.substring(1);
            }
            arg[1 + i] = elem;
            provided = provided + ((i > 0) ? "," : "") + elem;
            if (elem.contains("a")) {
              elemList.add(elem);
            }
          }

          // Sort expected output list for easy comparison
          Collections.sort(elemList);
          String expected = String.join(",", elemList);
          ...
          if (response.equals(expected)) {
            ...
            testFailed("We put " + numElements + " random strings ('" + provided
                + "') into an RDD and called filter() with the predicate that the string contains 'a'. We expected:\n\n"
                + expected + "\n\nbut got back:\n\n" + dump(response.getBytes()));
          }
        } catch (ConnectException ce) {
          testFailed(
              "Connection to Flame coordinator at localhost:9000 failed. Please verify if the coordinator is running.");
        }
      } catch (Exception e) {
        testFailed("An exception occurred: " + e, false);
        e.printStackTrace();
