	...
	public static void run(FlameContext ctx, String args[]) throws Exception {
		if (args[0].equals("count")) {
			...
			List<FlamePair> out = ctx.parallelize(list)
					.flatMapToPair(
							s -> Arrays.asList(new FlamePair[] { new FlamePair("" + s.charAt(0), s.substring(1)) }))
					.collect();
			Collections.sort(out);
			String ret = "";
			for (FlamePair p : out)
			...
			List<String> out = ctx.parallelize(list).mapToPair(s -> new FlamePair(s.split(",")[0], s.split(",")[1]))
					.flatMap(p -> Arrays.asList(new String[] { p._1() + p._2(), p._2() + p._1() })).collect();
			Collections.sort(out);
			String ret = "";
			for (String s : out)
				ret = ret + (ret.equals("") ? "" : ",") + s;
			...
			for (int i = 1; i < args.length; i += 2)
				list.add(args[i] + "," + args[i + 1]);
			List<FlamePair> out = ctx.parallelize(list).mapToPair(s -> new FlamePair(s.split(",")[0], s.split(",")[1]))
					.flatMapToPair(p -> Arrays.asList(new FlamePair[] { new FlamePair(p._1(), "" + p._1().length()),
							new FlamePair(p._2(), "" + p._2().length()) }))
					.collect();
			Collections.sort(out);
			String ret = "";
			for (FlamePair p : out)
